 ...
 *
 */
// HeapPage既是HeapFile中的一页，也是BufferPool中页的实现(pageId, 硬盘读取的数据)
// page内部为头部和元组，而头部使用位图来标识元组
public class HeapPage implements Page {
    ...
    private int getNumTuples() {
        // some code goes here
        if (this.numSlots != 0) return this.numSlots;
        int numTuples = (int)Math.floor(BufferPool.PAGE_SIZE*8/(this.td.getSize()*8+1));
        return numTuples;
    }
    ...
    public void deleteTuple(Tuple t) throws DbException {
        // some code goes here
        if (this.getNumEmptySlots() == this.numSlots) throw new DbException("page is empty");
        if (!t.getRecordId().getPageId().equals(this.pid)) throw new DbException("tuple is not in this page");
    ...
    public void insertTuple(Tuple t) throws DbException {
        ...
        if (!this.td.equals(t.getTupleDesc())) throw new DbException("tupledesc is mismatch");
    ...
    public boolean isSlotUsed(int i) {
        ...
        int byteNum = i / 8;
        int posInByte = i % 8;
        if (byteNum >= this.header.length) {
            throw new IllegalArgumentException("invalid i");
        }
        return (this.header[byteNum] & (1 << posInByte)) > 0;
    }
    ...
    private void markSlotUsed(int i, boolean value) {
        // some code goes here
        // TODO 理解位运算
        byte bit = this.header[i / 8];
        if (value) {
            this.header[i/8] = (byte) (bit | (1 << (i % 8)));
        } else {
            this.header[i/8] = (byte) (bit ^ (1 << (i % 8)));
        }
    }
    ...
    public Iterator<Tuple> iterator() {
        ...
        return new Iterator<Tuple>() {
            ...
            public boolean hasNext() {
                return (usedNum > 0 && index < getNumTuples());
            }
