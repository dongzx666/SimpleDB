...
public class Join_MergeSort extends Operator {
     ...
     */
    public Join_MergeSort(JoinPredicate p, DbIterator child1, DbIterator child2) {
        // some code goes here
        this.p = p;
        this.child1 = child1;
        this.child2 = child2;
    ...
    private void getJoin (int leftSize, int rightSize) {
        ...
        // 1.第一部分, sort, GREATER_THAN用降序排列, LESS_THAN和EQUALS用升序排列
...
//        JoinPredicate lessThan = new JoinPredicate(p.getField1(), Predicate.Op.LESS_THAN, p.getField2());
        ...
        while (left < l && right < r) {
            ...
            if (p.filter(ltp, rtp)) {
                ...
                Tuple tp = this.mergeJoinTuple(ltp, rtp);
                this.tuples.add(tp);
                right++;
                if (right >= r && left < l) {
                    left++;
