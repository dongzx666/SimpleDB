...
public class HeapFile implements DbFile {
    ...
    public ArrayList<Page> insertTuple(TransactionId tid, Tuple t)
        ...
        // some code goes here
        // 找有空插槽的page, 否则创建新page来操作
        ArrayList<Page> resList = new ArrayList<Page>();
        if (t == null) return resList;

        boolean hasEmptySlotPage = false;
        int i = 0;
        for (; i < this.numPages(); i++) {
            ...
            if (page.getNumEmptySlots() != 0) {
                ...
                resList.add(page);
                this.writePage(page);
                hasEmptySlotPage = true;
                break;
            }
        }

        if (this.numPages() == 0 || !hasEmptySlotPage) {
