...
// Aggregate指的数据库中的聚合, 包括COUNT，SUM，AVG，MIN，MAX等, 仅支持单列
public class Aggregate extends Operator {
    ...
    public Aggregate(DbIterator child, int afield, int gfield, Aggregator.Op aop) {
        ...
        Type atype = child.getTupleDesc().getFieldType(this.afield);
        // fix bug 如果gfield为-1会造成数组越界
        Type gtype = this.gfield == -1 ? null : this.child.getTupleDesc().getFieldType(this.gfield);
    ...
    public TupleDesc getTupleDesc() {
	    ...
	    String aname = this.child.getTupleDesc().getFieldName(this.afield);
	    String gname = this.child.getTupleDesc().getFieldName(this.gfield);

	    if (this.gfield != Aggregator.NO_GROUPING) {
            typeAr = new Type[]{this.child.getTupleDesc().getFieldType(this.gfield), Type.INT_TYPE};
            fieldAr = new String[]{gname, aname};
        } else {
            typeAr = new Type[]{Type.INT_TYPE};
            fieldAr = new String[]{aname};
        }
